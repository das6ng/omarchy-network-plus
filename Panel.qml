...
Panel {
  ...
  function refresh(scanWifi) {
    ...
    if (!allIfacesProc.running) allIfacesProc.running = true
    if (!dnsProc.running) {
  ...
  // Parses network-status-all.sh output: default-route keys up to a "---"
  // line, then one `interface` line per iface followed by its `ipv4`/`ipv6`
  // address lines.
  function updateAllInterfaces(raw) {
...
")
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (!inInterfaces) {
        if (key === "iface") defaultIface = rest
        continue
      }
  ...
  function ifaceEntry(name) {
    for (var i = 0; i < allInterfaces.length; i++) {
      if (allInterfaces[i].name === name) return allInterfaces[i]
    }
    return null
  }
  ...
  // Pulls every interface with its full v4/v6 address list in one shot.
  Process {
    id: allIfacesProc
    command: ["bash", root.allIfacesScript]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateAllInterfaces(text)
    }
  }

  // Pulls everything we want about the active route's interface in one shot.  // Pulls everything we want about the active route's interface in one shot.
  Process {
    id: detailsProc
  ...
  // Interface list changes rarely; 5s keeps it fresh without spamming `ip`.
  Timer {
    id: allIfacesPoll
    interval: 5000
    repeat: true
    running: root.opened
    onTriggered: if (!allIfacesProc.running) allIfacesProc.running = true
  }
  ...
  KeyboardPanel {
    ...
    Column {
      ...
      // Interface selector: which iface the address list below describes.
      PanelSeparator {
        visible: root.allInterfaces.length > 1
        foreground: root.bar.foreground
      }
      ...
      Column {
        ...
        Row {
          ...
          readonly property int count: Math.max(1, root.allInterfaces.length)
          readonly property real cellWidth: (width - spacing * (count - 1)) / count

          // Wrapper takes modelData from the Repeater's delegate context;
          // same shape as the band row above.
          Repeater {
            model: root.allInterfaces

            delegate: Item {
              required property var modelData
              width: ifaceRow.cellWidth
              height: ifacePill.implicitHeight

              InterfacePill {
                id: ifacePill
                ifaceName: modelData.name
                width: parent.width
              }
            }
          }
      ...
      // Connection details: transfer metrics first, then IP/Gateway.      // Connection details: transfer metrics first, then IP/Gateway.
      Column {
  ...
  // One interface pill. `active` marks the interface whose addresses the
  // details grid shows.
  component InterfacePill: Button {
    id: pill
    ...
    active: root.selectedInterface === ifaceName

    onClicked: root.selectedInterface = ifaceName
  ...
  // A single Wi-Fi network entry. Collapses to a one-line pill normally;  // A single Wi-Fi network entry. Collapses to a one-line pill normally;
